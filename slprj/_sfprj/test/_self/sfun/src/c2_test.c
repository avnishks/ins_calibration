...
static void set_sim_state_c2_test(SFc2_testInstanceStruct *chartInstance, const mxArray *c2_st);
...
static void c2_emlrt_marshallIn(SFc2_testInstanceStruct *chartInstance, const mxArray *c2_y, const char_T *c2_name, real_T c2_b_y[3]);
static uint8_T c2_b_emlrt_marshallIn(SFc2_testInstanceStruct *chartInstance, const mxArray *c2_b_is_active_c2_test, const char_T *c2_name);
...
chartInstance->c2_is_active_c2_test = c2_b_emlrt_marshallIn(chartInstance, sf_mex_dup(sf_mex_getcell(c2_u, 1)), "is_active_c2_test");
sf_mex_destroy(&c2_u);
...
static uint8_T c2_b_emlrt_marshallIn(SFc2_testInstanceStruct *chartInstance, const mxArray *c2_b_is_active_c2_test, const char_T *
c2_name)
{
uint8_T c2_y;
uint8_T c2_u0;
sf_mex_import(c2_name, sf_mex_dup(c2_b_is_active_c2_test), &c2_u0, 1, 3, 0U, 0, 0U, 0);
c2_y = c2_u0;
...
static mxArray *sf_get_sim_state_info_c2_test(void)
{
   const char *infoFields[] = {"chartChecksum", "varInfo"};
   mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
   const char *infoEncStr[] = {
   "100 S1x2'type','srcId','name','auxInfo'{{M[1],M[5],T\"y\",},{M[8],M[0],T\"is_active_c2_test\",}}"
   };
...
static void chart_debug_initialization(SimStruct *S, unsigned int fullDebuggerInitialization)
{
   if(!sim_mode_is_rtw_gen(S)) {
        SFc2_testInstanceStruct *chartInstance;
        chartInstance = (SFc2_testInstanceStruct *) ((ChartInfoStruct *)(ssGetUserData(S)))->chartInstance;
...
static mxArray* sf_internal_get_sim_state_c2_test(SimStruct* S)
{
    ...
    prhs[2] = (mxArray*) get_sim_state_c2_test((SFc2_testInstanceStruct*)chartInfo->chartInstance); /* raw sim ctx */
    prhs[3] = sf_get_sim_state_info_c2_test(); /* state var info */
...
static void sf_internal_set_sim_state_c2_test(SimStruct* S, const mxArray *st)
{
    ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
    mxArray *plhs[1] = {NULL};
    mxArray *prhs[4];
    ...
    set_sim_state_c2_test((SFc2_testInstanceStruct*)chartInfo->chartInstance, mxDuplicateArray(plhs[0]));
    mxDestroyArray(plhs[0]);
}

static mxArray* sf_opaque_get_sim_state_c2_test(SimStruct* S)
{
...
static void mdlProcessParameters_c2_test(SimStruct *S)
{
   ...
   if(sf_machine_global_initializer_called()) {
       initialize_params_c2_test((SFc2_testInstanceStruct*)(((ChartInfoStruct *)ssGetUserData(S))->chartInstance));
   }
}
...
static void mdlSetWorkWidths_c2_test(SimStruct *S)
{
 ...
 if(sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
     int_T chartIsInlinable =
               (int_T)sf_is_chart_inlinable("test","test",2);
     ssSetStateflowIsInlinable(S,chartIsInlinable);
		ssSetRTWCG(S,sf_rtw_info_uint_prop("test","test",2,"RTWCG"));
      ssSetEnableFcnIsTrivial(S,1);
      ssSetDisableFcnIsTrivial(S,1);
		ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop("test","test",2,"gatewayCannotBeInlinedMultipleTimes"));
     if(chartIsInlinable) {            
           ssSetInputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
             sf_mark_chart_expressionable_inputs(S,"test","test",2,1);
             sf_mark_chart_reusable_outputs(S,"test","test",2,1);
     }
     sf_set_rtw_dwork_info(S,"test","test",2);
...
{
   if(sim_mode_is_rtw_gen(S)) {
      sf_write_symbol_mapping(S, "test", "test",2);
...
{
 SFc2_testInstanceStruct *chartInstance;
 chartInstance = (SFc2_testInstanceStruct *)malloc(sizeof(SFc2_testInstanceStruct));
